...
| `IdleChecks.cs` | Idle autonomy: the quiet spell before she takes over, terrain-sampled destinations with window tops weighted up, routes found over a navigation graph of the solids themselves (walk, climb, up-dash, drop, and leap edges), stroll legs that climb to elevated spots and stay strolls, climbing a window to sit on it, neutral-jumping one taller than her stamina, hanging off the screen's edge wall and the lip of a window, hopping the one-pixel seam between monitors, climbing out of a hole, reaching a floating window by up-dash or by leaping from any neighbouring wall, the watchdog giving up on what cannot work, napping through something fullscreen, the jellyfish errand, and the dash -- earned on a proven-clear stroll, refused wherever a dash could move a window |
