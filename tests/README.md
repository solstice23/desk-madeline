...
| `IdleChecks.cs` | Idle autonomy: the quiet spell before she takes over, walking and jumping to where she decided to go, climbing a window to sit on it, neutral-jumping one taller than her stamina, hanging off the screen's edge wall and the lip of a window, hopping the one-pixel seam between monitors, climbing out of a hole, reaching a floating window by up-dash or by leaping from the edge wall, the watchdog giving up on what cannot work, napping through something fullscreen, the jellyfish errand, and the dash -- earned on a proven-clear stroll, refused wherever a dash could move a window |
