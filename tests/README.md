...
| `IdleChecks.cs` | Idle autonomy: the quiet spell before she takes over, walking and jumping to where she decided to go, climbing a window to sit on it, neutral-jumping one taller than her stamina, hanging off the screen's edge wall, the watchdog giving up on what cannot work, napping through something fullscreen, the jellyfish errand, and the dash -- earned on a proven-clear stroll, refused entirely in kevin mode |
